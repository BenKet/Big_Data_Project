...
def get_color(weight, vmin, vmax):
    ...
    cmap = cm.RdYlGn  # Direct access to colormap
...
def main():
    ...
    with tabs[0]:
        ...
        # Initialize map
        map_center = [52.50306, 13.42470]  # Example center (Berlin)
        mymap = folium.Map(location=map_center, zoom_start=14)

        # Fetch and display existing edges
        edges = fetch_edges()
    ...
    with tabs[1]:
        ...
        if st.button("Generate Route"):
            ...
            st.markdown("**API Request to Insert Route:**")
            st.code(json.dumps(api_request, indent=4))

            # Save the last generated API call to a JSON file
            with open("last_api_call.json", "w") as f:
                json.dump(api_request, f, indent=4)

    with tabs[2]:
        st.header("Last API Call")
        try:
            with open("last_api_call_received.json", "r") as f:
                last_api_call = json.load(f)
                st.json(last_api_call)
